	...
	// if key is found in the array, return the location, otherwise return -1  
{
	...
	for(int i = hi / 2; i != M || i == 0; ++i) {
		if(iterations > 27) {
			printf("Broke at key: %d", key);
			break;
		}

		if(x[i] == key) { // Got a hit
			return i;
		}
		else if(x[i] < key) { // We are too low, splitting array into lower half
			lo = i;
			i = (lo + hi) / 2;
		} 
		else if (x[i] > key) { // We are too high, splitting array into the lower half
			hi = i;
			i = (lo + hi) / 2;
		}
		++iterations;
